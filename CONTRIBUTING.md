...
Keep changes focused on the native Astra-led worker workflow, installation reliability, useful task contracts and review evidence. Preserve the user's root model, existing Router and security boundaries.
...
Record every user-visible or behavioral change in both files: a dated, append-only
entry in `HISTORY.md` and a version bullet in `CHANGELOG.md`. Never rewrite an
existing `HISTORY.md` entry.
